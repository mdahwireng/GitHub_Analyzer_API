...
def dev_to_prod(dev_url, prod_url, plural_api, index_cols, token, table=None):
    ...
    dev_data = retrieve_table_data(url=dev_url+"/"+plural_api, index_cols=index_cols, token=token["prod"])
    
    for k,v in dev_data.items():
        dev_entry_id = v["id"]
        
        if table:
            v["attributes"]["trainee"] = get_trainee_relation_id(base_url=dev_base_url, table=table, entry_id=dev_entry_id, token=token["prod"])
...
if __name__ == "__main__":
    ...
    if strapi_token:
        ...
        dev_url = "https://cms.10academy.org/api"
